...
| D4 | `terraform apply` em produção: aprovação automática no merge, ou um clique manual de confirmação? | **✅ Clique manual** (decidido 02/09/2026), via GitHub Environments com o próprio owner como *required reviewer* do ambiente `production` | Dinheiro real, operador único, sem colega para revisar o PR antes. Um ambiente do GitHub com aprovação obrigatória funciona como o "tem certeza?" que substitui a segunda pessoa — e fica registrado em log. `plan` continua automático em todo PR. **Revisado em 08/09/2026:** o portão saiu do *deploy do frontend* e continua no `terraform apply`, no deploy da API e na ingestão. O frontend sincroniza arquivo estático num bucket — não toca banco, não troca imagem de serviço, e reverter é publicar de novo. Em troca, o cron horário somado à fila de concorrência fazia um run parado esperando aprovação travar todos os seguintes: chegou a seis horas de fila com o build que interessava sem começar. O portão passou a custar disponibilidade sem comprar segurança. |
